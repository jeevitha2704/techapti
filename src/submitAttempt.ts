// functions/src/submitAttempt.ts
...
/**
 * Callable function submitAttempt
 * Input: { attemptId, quizId, answers: [{ questionId, selectedIndex }] }
 * Behavior:
 *  - require auth
 *  - load server-side quiz doc (contains questions with answerIndex)
 *  - validate question IDs
 *  - ensure attempt exists and belongs to caller
 *  - compute details[], score, durationSeconds
 *  - write results into attempts/{attemptId} (endTime, durationSeconds, score, total, details, status)
 *  - write a summary into quizzes/{quizId}/attempts/{attemptId} for fast listing
